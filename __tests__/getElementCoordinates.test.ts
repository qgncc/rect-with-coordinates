...
test("Should return correct coordinates relative to the top left corner of a rectangle", () => {
  ...
  expect(element.transformViewportCoordinates([150, 150])).toStrictEqual([
    140, 130,
  ]);
});
...
test("Should return correct out of bounds coordinates relative to the top left corner of a rectangle", () => {
  ...
  expect(
    element.transformViewportCoordinates([5, 10], { noOutOfBounds: false })
  ).toStrictEqual([-5, -10]);
  expect(
    element.transformViewportCoordinates([250, 250], { noOutOfBounds: false })
...
test('Should return correctly "cropped out" coordinates to keep them inside of bounds', () => {
  ...
  expect(
    element.transformViewportCoordinates([5, 10], {
      noOutOfBounds: true,
    })
  ).toStrictEqual([0, 0]);
  expect(
    element.transformViewportCoordinates([250, 250], {
      noOutOfBounds: true,
    })
...
test("Should return correct coordinates relatively to another origin", () => {
  ...
  expect(
    element.transformViewportCoordinates([150, 150], {
      customOrigin: [12, 43],
    })
